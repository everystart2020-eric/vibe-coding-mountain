...
**에러를 만날 때마다 실력이 늡니다. 에러는 성장의 디딤돌!** 🌿`,
  ...
  },
  {
    id: "lovable",
    name: "Lovable 바이브코딩산",
    subtitle: "북한산 — 코드 없이 서비스를 만드는 4주 여정",
    emoji: "💜",
    difficulty: "초급",
    description: "자연어 한 문장으로 실제 웹 서비스를 만드는 바이브코딩! Lovable + Supabase + Gemini를 활용해 4주 만에 나만의 서비스를 완성합니다.",
    ...
    stages: [
      {
        ...
        title: "바이브코딩이란?",
        description: "코드 없이 서비스를 만드는 새로운 패러다임을 이해합니다.",
        prerequisites: [],
        content: `# 바이브코딩이란?

## 핵심 개념

**바이브코딩(Vibe Coding)**은 자연어(우리가 평소 쓰는 말)로 AI에게 지시해 실제 동작하는 웹 서비스를 만드는 방식입니다.

> "코드를 직접 작성하지 않아도, 내가 원하는 서비스를 만들 수 있다."
...
1. **PRD 작성** — 서비스 기획서를 AI가 이해할 수 있는 형태로 작성
2. **Lovable 활용** — 첫 화면부터 완성 서비스까지
3. **Supabase 연동** — 데이터베이스 없이 백엔드 구성
4. **Gemini API** — AI 기능을 내 서비스에 추가
...
- 비개발자도 기술 창업 가능`,
        quiz: [
          {
            question: "바이브코딩에서 '인간'의 주된 역할은 무엇인가요?",
            options: [
              "직접 HTML/CSS 코드 작성",
              "아이디어·기획·방향 결정과 피드백",
              "서버 인프라 관리",
            ...
            correct: 1,
            explanation: "바이브코딩에서 인간은 아이디어와 기획, 방향을 결정하고 피드백을 줍니다. 코드 작성, UI 생성, 버그 수정은 AI가 담당합니다."
          },
          {
            question: "바이브코딩의 가장 큰 장점은 무엇인가요?",
            options: [
              "전통적인 코딩보다 항상 더 안정적인 코드 생성",
              "아이디어를 당일 프로토타입으로 빠르게 구현 가능",
              "복잡한 알고리즘을 자동으로 최적화",
              "유지보수 비용이 전혀 없음"
            ],
            correct: 1,
            explanation: "바이브코딩의 핵심 장점은 속도입니다. 아이디어를 당일 프로토타입으로 만들 수 있고, 비개발자도 MVP를 출시할 수 있습니다."
          },
          {
            question: "이번 커리큘럼에서 활용하는 도구 조합은?",
            options: [
              "React + Node.js + MongoDB",
              "Vue + Firebase + OpenAI",
              "Lovable + Supabase + Gemini",
              "Next.js + Vercel + ChatGPT"
            ],
            correct: 2,
            explanation: "이번 커리큘럼은 Lovable(UI 생성) + Supabase(백엔드/DB) + Gemini API(AI 기능)의 조합을 사용합니다."
          }
        ]
      },
      {
        id: "prd-writing",
        title: "PRD 작성하기",
        description: "AI가 이해할 수 있는 서비스 기획서(PRD)를 작성하는 방법을 배웁니다.",
        prerequisites: ["what-is-vibecoding"],
        content: `# PRD 작성하기

## PRD란?

**PRD(Product Requirements Document)**는 서비스 기획서입니다. AI에게 무엇을 만들어야 하는지 정확히 전달하기 위한 문서입니다.
...
2. **목적** — 이 서비스가 해결하는 문제
3. **타겟 사용자** — 누가 사용하는가
4. **핵심 기능** — 반드시 있어야 할 기능 (P0)
5. **추가 기능** — 있으면 좋은 기능 (P1/P2)
6. **디자인 방향** — 색상, 분위기, 레퍼런스
7. **기술 스택** — 사용할 도구 (Lovable, Supabase 등)
...
| 나쁜 PRD | 좋은 PRD |
|---------|---------|
| "멋진 앱 만들어줘" | "20-30대 직장인을 위한 점심 메뉴 추천 앱" |
| 기능 목록만 나열 | 목적 + 사용자 + 기능 + 디자인 모두 포함 |
| 모호한 표현 | 구체적인 예시 포함 |
| 우선순위 없음 | P0/P1/P2로 우선순위 명시 |
        ...
        quiz: [
          {
            ...
            correct: 1,
            explanation: "P0는 Priority 0으로, 서비스가 동작하기 위해 반드시 있어야 할 핵심 기능입니다. P1은 있으면 좋은 기능, P2는 나중에 추가할 기능입니다."
          },
          {
            question: "좋은 PRD의 특징이 아닌 것은?",
            options: [
              "목적과 타겟 사용자가 명확함",
              "기능에 우선순위가 있음",
              "최대한 많은 기능을 나열",
            ...
            correct: 2,
            explanation: "좋은 PRD는 기능을 많이 나열하는 것이 아니라, 핵심 기능을 우선순위에 따라 명확하게 정의합니다. 처음에는 P0(핵심 기능)에 집중해야 합니다."
          },
          {
            question: "GPT로 PRD를 작성할 때 올바른 순서는?",
            options: [
              "Lovable 실행 → GPT 요청 → PRD 작성",
              "PRD 작성 → GPT 검토 → Lovable 실행",
              "아이디어 설명 → GPT에 PRD 요청 → 검토 → Lovable 붙여넣기",
              "기능 목록 작성 → 디자인 결정 → GPT 확인"
            ],
            correct: 2,
            explanation: "올바른 순서: 아이디어를 GPT에 설명 → PRD 형식으로 정리 요청 → 내용 검토 및 수정 → Lovable에 붙여넣기입니다."
          }
        ]
      },
      {
        id: "first-screen",
        title: "Lovable 첫 화면 만들기",
        description: "Lovable에서 첫 번째 화면을 생성하고 P0 기능을 구현합니다.",
        prerequisites: ["prd-writing"],
        content: `# Lovable 첫 화면 만들기

## Lovable이란?

**Lovable(lovable.dev)**은 자연어 프롬프트로 React 기반 웹 앱을 생성하는 AI 도구입니다.

> "만들고 싶은 것을 말하면, 코드가 완성됩니다."
...
처음 프롬프트에는 **핵심 기능(P0)만** 요청합니다.

\`\`\`
❌ 나쁜 예:
"로그인, 회원가입, 메인 피드, 좋아요, 댓글, 검색,
알림, 프로필 수정, 다크모드까지 모두 만들어줘"

✓ 좋은 예:
"음식 리뷰 앱의 메인 화면을 만들어줘.
카드 형태로 음식 사진과 별점을 보여주는 레이아웃.
상단에 카테고리 필터 버튼(한식, 중식, 일식)."
\`\`\`
...
- "모바일에서도 잘 보이게 반응형으로 수정해줘"`,
        quiz: [
          {
            question: "Lovable 첫 번째 프롬프트에서 권장하는 접근 방식은?",
            options: [
              "모든 기능을 한번에 요청",
              "P0 핵심 기능에만 집중",
              "디자인만 먼저 요청",
            ...
            correct: 1,
            explanation: "첫 프롬프트는 P0(핵심 기능)에만 집중해야 합니다. 모든 기능을 한번에 요청하면 AI가 제대로 구현하기 어렵고, 결과물도 복잡해집니다."
          },
          {
            question: "Lovable 워크플로우의 올바른 순서는?",
            options: [
              "확인 → 생성 → 수정 → 반복",
              "생성 → 확인 → 수정 → 반복",
              "수정 → 생성 → 확인 → 반복",
            ...
            correct: 1,
            explanation: "Lovable 워크플로우: 1)생성(프롬프트 입력) → 2)확인(미리보기) → 3)수정(채팅 요청) → 4)반복입니다."
          },
          {
            question: "Lovable은 어떤 기술 기반으로 앱을 생성하나요?",
            options: [
              "Vue.js",
              "Angular",
              "React",
            ...
            correct: 2,
            explanation: "Lovable은 React 기반의 웹 앱을 생성합니다. 따라서 생성된 코드는 React 컴포넌트 구조로 되어 있습니다."
          }
        ]
      },
      {
        id: "phase-knowledge",
        title: "Phase 분할 & Knowledge",
        description: "P0→P1→P2→P3 단계별 개발과 Knowledge 기능으로 일관성을 유지합니다.",
        prerequisites: ["first-screen"],
        content: `# Phase 분할 & Knowledge

## Phase(단계) 분할이란?

서비스를 **한번에 만들려고 하지 마세요.** 단계별로 나눠서 만들어야 품질이 높아집니다.
...
| Phase | 의미 | 예시 |
|-------|------|------|
| P0 | 없으면 서비스 불가 | 메인 화면, 핵심 기능 |
| P1 | 있으면 훨씬 좋음 | 검색, 필터 |
| P2 | 편의 기능 | 다크모드, 즐겨찾기 |
| P3 | 미래 기능 | AI 추천, 고급 분석 |
...
**Knowledge**는 Lovable에게 프로젝트의 규칙을 알려주는 기능입니다.

### Knowledge에 포함할 내용 6가지

1. **색상 팔레트** — 메인 색상, 보조 색상
2. **폰트 규칙** — 제목/본문 폰트 크기
3. **컴포넌트 스타일** — 버튼, 카드 모양
4. **브랜드 톤** — 친근함/전문적/밝음
5. **금지 사항** — 쓰지 않을 색상이나 패턴
6. **레이아웃 규칙** — 여백, 정렬 방식
...
\`\`\`
Lovable 좌측 패널 → Knowledge 탭 → 내용 입력

예시:
"이 앱의 메인 색상은 보라색(#7C3AED)입니다.
버튼은 항상 둥근 모서리(rounded-full)를 사용합니다.
폰트는 Pretendard를 사용합니다."
\`\`\``,
        quiz: [
          {
            question: "P0 기능의 정의로 가장 올바른 것은?",
            options: [
              "있으면 편리한 부가 기능",
              "미래에 추가할 고급 기능",
              "없으면 서비스 자체가 불가능한 핵심 기능",
              "디자인 관련 기능"
            ],
            correct: 2,
            explanation: "P0는 없으면 서비스 자체가 성립하지 않는 핵심 기능입니다. 예: 쇼핑몰의 상품 목록, SNS의 피드 표시."
          },
          {
            question: "Lovable의 Knowledge 기능을 사용하는 주된 이유는?",
            options: [
              "코드 자동 최적화",
              "일관된 디자인과 스타일 유지",
              "데이터베이스 연결",
            ...
            correct: 1,
            explanation: "Knowledge는 색상, 폰트, 컴포넌트 스타일 등 프로젝트 규칙을 저장해 수정할 때마다 일관된 디자인이 유지되도록 합니다."
          },
          {
            question: "4주 커리큘럼에서 Week 1에 집중해야 할 Phase는?",
            options: [
              "P3 — AI 기능",
              "P2 — 편의 기능",
              "P1 — 있으면 좋은 기능",
            ...
            correct: 3,
            explanation: "Week 1은 P0(핵심 기능) 완성에 집중합니다. 메인 화면과 서비스의 핵심 기능을 먼저 완성한 후 P1, P2를 추가합니다."
          }
        ]
      },
      {
        id: "ux-ui-design",
        title: "UX/UI 설계 — UX Pilot 활용",
        description: "UX와 UI의 차이를 이해하고 UX Pilot로 전문적인 디자인을 만듭니다.",
        prerequisites: ["phase-knowledge"],
        content: `# UX/UI 설계 — UX Pilot 활용

## UI vs UX 차이

| 구분 | UI (User Interface) | UX (User Experience) |
|------|--------------------|--------------------|
| 정의 | 화면에 보이는 것 | 사용자가 느끼는 경험 |
| 예시 | 버튼 색상, 폰트 | 버튼 찾기 쉬운 위치 |
| 질문 | "예쁜가?" | "편한가?" |
| 목표 | 시각적 완성도 | 사용성과 만족감 |

> 좋은 서비스는 UI(예쁨)와 UX(편함)를 모두 갖춰야 합니다.

---

## UX Pilot이란?

**UX Pilot**은 AI가 UX/UI 디자인을 도와주는 도구입니다.
...
1. **목적 입력** — 서비스 목적과 타겟 사용자 설명
2. **화면 선택** — 만들 화면 선택 (메인, 상세, 목록 등)
3. **스타일 설정** — 색상 방향, 분위기 선택
4. **와이어프레임 생성** — AI가 레이아웃 초안 생성
5. **Lovable 전달** — 생성된 디자인을 Lovable 프롬프트로 변환
...
"보라색 계열의 미니멀 디자인.
카드는 흰색 배경에 그림자 효과.
제목은 24px 굵게, 본문은 14px 회색.
버튼은 보라색 배경에 흰색 텍스트, 둥근 모서리."
\`\`\`
...
- [ ] 데스크탑(1280px)에서 확인
- [ ] 텍스트가 너무 작지 않은지 확인
- [ ] 버튼이 손가락으로 탭하기 충분한 크기인지 확인`,
        quiz: [
          {
            question: "UX(User Experience)를 가장 잘 설명하는 것은?",
            options: [
              "버튼의 색상과 모양",
              "화면에 보이는 시각적 요소",
              "사용자가 서비스를 사용하며 느끼는 전체적인 경험",
              "앱의 로딩 속도"
            ],
            correct: 2,
            explanation: "UX는 사용자가 서비스를 사용하는 전체 경험입니다. '이 서비스는 편한가? 원하는 것을 쉽게 찾을 수 있는가?'가 UX의 핵심 질문입니다."
          },
          {
            question: "UX Pilot의 주요 활용 목적은?",
            options: [
              "데이터베이스 설계",
              "AI 기능 추가",
              "UX/UI 디자인 초안 생성 지원",
            ...
            correct: 2,
            explanation: "UX Pilot은 와이어프레임 자동 생성, 색상 팔레트 추천, 컴포넌트 레이아웃 제안 등 UX/UI 디자인 작업을 AI가 지원합니다."
          },
          {
            question: "모바일 반응형 디자인에서 기준이 되는 화면 너비는?",
            options: [
              "320px",
              "375px",
              "414px",
            ...
            correct: 1,
            explanation: "모바일 반응형 디자인의 기준 너비는 375px입니다(iPhone 기준). 이 크기에서 레이아웃이 제대로 표시되는지 먼저 확인합니다."
          }
        ]
      },
      {
        id: "supabase-backend",
        title: "백엔드 연결 — Supabase",
        description: "Supabase로 데이터베이스를 연결하고 실제 데이터를 저장·불러옵니다.",
        prerequisites: ["ux-ui-design"],
        content: `# 백엔드 연결 — Supabase
...
**Supabase**는 오픈소스 Firebase 대안으로, PostgreSQL 기반의 BaaS(Backend as a Service)입니다.

### Lovable Cloud vs Supabase

| 기능 | Lovable Cloud | Supabase |
|------|--------------|---------|
| 데이터 영구 저장 | ❌ 새로고침 시 초기화 | ✅ 영구 저장 |
| 사용자 간 데이터 공유 | ❌ | ✅ |
| 로그인/회원가입 | ❌ | ✅ |
| 비용 | 무료 | 무료 (소규모) |
...
2단계: Lovable에서 Supabase 연결
   → Lovable 우측 패널 → Supabase Connect
   → Project URL과 API Key 입력

3단계: 테이블 생성 요청
   → Lovable 채팅: "리뷰 데이터를 저장할 테이블 만들어줘"
\`\`\`
...
\`\`\`
"[기능명] 데이터를 Supabase에 저장해줘.
저장할 정보: [필드1, 필드2, 필드3]
- [필드1]: [타입] — [설명]
- [필드2]: [타입] — [설명]"

예시:
"음식 리뷰 데이터를 Supabase에 저장해줘.
저장할 정보: 음식 이름, 별점, 리뷰 내용, 작성일
- 음식 이름: 텍스트 — 리뷰한 음식 이름
- 별점: 숫자(1-5) — 1에서 5 사이
- 리뷰 내용: 긴 텍스트 — 상세 후기"
\`\`\`
...
**RLS**는 데이터 접근 권한을 행(Row) 단위로 제어합니다.

> "내가 쓴 글은 나만 수정할 수 있다"

Lovable에게: "RLS를 설정해서 본인 데이터만 수정/삭제 가능하게 해줘"`,
        quiz: [
          {
            question: "Supabase를 사용하는 주된 이유는?",
            options: [
              "UI 디자인 자동화",
              "데이터 영구 저장과 사용자 간 공유",
              "코드 오류 자동 수정",
            ...
            correct: 1,
            explanation: "Lovable Cloud는 새로고침 시 데이터가 사라지지만, Supabase를 연결하면 데이터를 영구 저장하고 사용자 간 공유할 수 있습니다."
          },
          {
            question: "RLS(Row Level Security)의 역할은?",
            options: [
              "데이터베이스 성능 향상",
              "UI 컴포넌트 보안",
              "행(Row) 단위로 데이터 접근 권한 제어",
              "API 요청 횟수 제한"
            ],
            correct: 2,
            explanation: "RLS는 '내가 쓴 글은 나만 수정/삭제 가능'처럼 행 단위로 데이터 접근을 제어합니다. 다른 사용자의 데이터를 보호하는 필수 보안 기능입니다."
          },
          {
            question: "Supabase의 기반 데이터베이스는?",
            options: [
              "MySQL",
              "MongoDB",
              "PostgreSQL",
            ...
            correct: 2,
            explanation: "Supabase는 PostgreSQL 기반의 BaaS(Backend as a Service)입니다. 강력한 관계형 데이터베이스를 코드 없이 사용할 수 있습니다."
          }
        ]
      },
      {
        id: "gemini-ai-feature",
        title: "AI 기능 추가 — Gemini API",
        description: "Gemini API를 연동해 내 서비스에 AI 기능을 추가합니다.",
        prerequisites: ["supabase-backend"],
        content: `# AI 기능 추가 — Gemini API

## Gemini API란?

**Gemini**는 Google의 AI 모델입니다. Gemini API를 사용하면 내 서비스에 AI 기능(텍스트 생성, 분석, 번역 등)을 추가할 수 있습니다.
...
| 규칙 | 설명 |
|------|------|
| ❌ 절대 금지 | 코드에 직접 API 키 작성 |
| ❌ 절대 금지 | GitHub에 API 키 올리기 |
| ✅ 올바른 방법 | Supabase Edge Function에 저장 |
| ✅ 올바른 방법 | 환경변수로 관리 |

> API 키가 유출되면 요금 폭탄을 맞을 수 있습니다!

---

## Gemini 무료 사용량

- **Gemini Flash**: 분당 15회 / 하루 1,500회 무료
- 개인 프로젝트에는 충분한 양
- 초과 시 과금 (소액)

---

## Edge Function으로 AI 기능 추가

\`\`\`
프롬프트 예시:
"Supabase Edge Function을 만들어서
사용자가 입력한 텍스트를 Gemini API로 분석해줘.
API 키는 환경변수 GEMINI_API_KEY로 저장해줘."
\`\`\`
...
## Plan Mode로 복잡한 기능 디버깅

기능이 잘 안 될 때:

\`\`\`
1. Lovable 우측 상단 "Plan Mode" 활성화
2. "왜 안 되는지 단계별로 분석해줘" 요청
3. AI가 문제 진단 후 수정 계획 제시
4. "실행해줘"로 적용
\`\`\`
...
- 추천 시스템`,
        quiz: [
          {
            question: "Gemini API 키를 올바르게 관리하는 방법은?",
            options: [
              "JavaScript 코드에 직접 작성",
              "README 파일에 기록",
              "Supabase Edge Function의 환경변수로 저장",
              "로컬 파일에 저장"
            ],
            correct: 2,
            explanation: "API 키는 Supabase Edge Function의 환경변수로 저장해야 합니다. 코드나 공개 저장소에 직접 작성하면 키가 유출되어 요금 폭탄을 맞을 수 있습니다."
          },
          {
            question: "Gemini Flash API의 무료 사용량은?",
            options: [
              "하루 100회",
              "분당 15회 / 하루 1,500회",
              "월 10,000회",
            ...
            correct: 1,
            explanation: "Gemini Flash는 분당 15회, 하루 1,500회까지 무료로 사용할 수 있습니다. 개인 프로젝트에는 충분한 양입니다."
          },
          {
            question: "Lovable의 Plan Mode를 사용하는 적절한 상황은?",
            options: [
              "색상을 변경할 때",
              "텍스트를 수정할 때",
              "복잡한 기능이 제대로 동작하지 않을 때",
              "새 프로젝트를 시작할 때"
            ],
            correct: 2,
            explanation: "Plan Mode는 복잡한 기능이 잘 안 될 때, AI가 문제를 단계별로 분석하고 수정 계획을 세운 후 실행하도록 하는 디버깅 모드입니다."
          }
        ]
      },
      {
        id: "login-admin-publish",
        title: "로그인·어드민·배포",
        description: "Supabase Auth로 로그인을 구현하고 서비스를 최종 배포합니다.",
        prerequisites: ["gemini-ai-feature"],
        content: `# 로그인·어드민·배포

## 로그인이 필요한가요?

모든 서비스에 로그인이 필요하지는 않습니다.

| 로그인 필요 | 로그인 불필요 |
|------------|-------------|
| 개인 데이터 저장 | 공개 정보만 표시 |
| 사용자별 다른 화면 | 모든 사람에게 같은 화면 |
| 글쓰기/수정/삭제 | 읽기만 가능 |

---

## Supabase Auth 4가지 방식

1. **이메일 + 비밀번호** — 가장 일반적
2. **소셜 로그인** — Google, GitHub, Apple 등
3. **매직 링크** — 이메일로 링크 발송 (비밀번호 없음)
4. **전화번호** — SMS 인증
...
어드민(관리자) 페이지에 포함할 4가지:

1. **대시보드** — 전체 통계 (가입자 수, 콘텐츠 수)
2. **콘텐츠 관리** — 글/상품 등록·수정·삭제
3. **사용자 관리** — 회원 목록, 차단
4. **설정** — 서비스 설정 변경
...
| 환경 | 용도 | 특징 |
|------|------|------|
| Preview | 개발 중 테스트 | 임시 URL, 변경 즉시 반영 |
| Staging | 출시 전 최종 확인 | 실제 환경과 동일 |
| Production | 실제 서비스 | 커스텀 도메인 연결 가능 |
| Custom Domain | 브랜딩 | myservice.com 형태 |
...
\`\`\``,
        quiz: [
          {
            question: "로그인 기능이 반드시 필요한 서비스 유형은?",
            options: [
              "날씨 정보를 보여주는 서비스",
              "뉴스 기사를 표시하는 서비스",
              "사용자가 글을 작성하고 본인 글만 수정할 수 있는 서비스",
              "식당 메뉴를 보여주는 서비스"
            ],
            correct: 2,
            explanation: "로그인은 개인 데이터 저장, 사용자별 다른 화면, 글쓰기/수정/삭제 기능이 필요할 때 사용합니다. 읽기 전용 공개 서비스는 로그인이 불필요합니다."
          },
          {
            question: "비밀번호 없이 이메일 링크만으로 로그인하는 방식은?",
            options: [
              "소셜 로그인",
              "이메일 + 비밀번호",
              "매직 링크",
            ...
            correct: 2,
            explanation: "매직 링크는 이메일로 로그인 링크를 발송하는 방식으로, 비밀번호가 필요 없습니다. 간단하고 보안성이 높습니다."
          },
          {
            question: "실제 서비스(Production)에 배포하기 전 가장 먼저 확인해야 할 것은?",
            options: [
              "커스텀 도메인 연결",
              "Google Analytics 설치",
              "모든 P0 핵심 기능이 정상 동작하는지 확인",
              "소셜 로그인 추가"
            ],
            correct: 2,
            explanation: "배포 전 가장 중요한 것은 P0(핵심 기능)이 모두 정상 동작하는지 확인하는 것입니다. 핵심 기능에 문제가 있으면 다른 것은 의미가 없습니다."
          }
        ]
      }
    ]
